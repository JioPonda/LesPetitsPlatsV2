...
function factorySearch(data) {
  ...
  let ingredientsArray2 =
    []; /** deuxiéme tableau avec les données pré-filtrer */

  for (let i = 0; i < data.length; i++) {
    ingredientsArray.push(
      data[i].ingredients
    ); /** pour chaque ingredient dans le JSON nous l'ajoutons dans le premier tableau*/
  }

  ingredientsArray =
    ingredientsArray.flat(); /** Mise a plat du premier tableau */

  for (let i = 0; i < ingredientsArray.length; i++) {
    ingredientsArray2.push(
      ingredientsArray[i].ingredient.toLowerCase()
    ); /** Pour chaque ingrédient dans le premier tableau nous l'ajoutons dans le deuxiémée 
        tableau en supprimant les majuscule */
  }

  let ingredientList = ingredientsArray2.filter(
    (x, i) => ingredientsArray2.indexOf(x) === i
  ); /** Nous créons une variable ou  nous filtrons les ingrédients 
    afin de retourner un tableau sans doublons */

  ingredientList.forEach((e) => {
    /** Pour chaqque element de nortre ingredientList */
    /** Element du DOM */
    ...
    suggestionIngredients.appendChild(
      pIngredient
    ); /** notre DIV suggestion-ingredient enfante de chaque lien crée précédemment */
  });
  /************************* APPLIANCE *************************************/
  ...
  for (let i = 0; i < data.length; i++) {
    appareilsArray.push(
      data[i].appliance.toLowerCase()
    ); /** pour chaque appareil dans le JSON nous l'ajoutons dans le tableau*/
  }

  appareilsArray = appareilsArray.flat(); /** Mise a plat du tableau */

  let appareilList = appareilsArray.filter(
    (x, i) => appareilsArray.indexOf(x) === i
  ); /** Nous créons une variable ou  nous filtrons les appareils 
    afin de retourner un tableau sans doublons */

  appareilList.forEach((e) => {
    /** Element du dom*/
    const suggestionAppareils = document.getElementById(
      "suggestions-appareils"
    );
    const pAppareils = document.createElement("p");
    pAppareils.setAttribute("id", "pAppareils");
    /** Texte et implémentation*/
    pAppareils.textContent =
      e; /** pour chaque lien crée nous ajoutons en texte les appareils */
    suggestionAppareils.appendChild(
      pAppareils
    ); /** notre DIV suggestion-ingédient enfante de chaque lien crée précédemment */
  });
  ...
  for (let i = 0; i < data.length; i++) {
    ustensilesArray.push(
      data[i].ustensils
    ); /** pour chaque ustensile dans le JSON nous l'ajoutons dans le tableau*/
  }
  ustensilesArray = ustensilesArray.flat(); /** Mise a plat du tableau */

  let ustensilslList = ustensilesArray.filter(
    (x, i) => ustensilesArray.indexOf(x) === i
  ); /** Nous créons une variable ou nous filtrons les ustensiles 
    afin de retourner un tableau sans doublons */

  ustensilslList.forEach((e) => {
    /** Element du dom*/
    const suggestionUstensils = document.getElementById(
      "suggestions-ustensiles"
    );
    const pUstensils = document.createElement("p");
    pUstensils.setAttribute("id", "pUstensils");
    /** Texte et implémentation*/
    pUstensils.textContent =
      e; /** pour chaque lien crée nous ajoutons en texte les ustensiles */
    suggestionUstensils.appendChild(
      pUstensils
    ); /** notre DIV suggestion-ingédient enfante de chaque lien crée précédemment */
  });
  ...
  const errorMessage =
    document.querySelector("#error-search"); /** message d'erreur */

  searchBar.addEventListener("keyup", function () {
    /** mise en place d'un écoute sur la barre de recherche*/ let arrayCard =
      [];
    let searchValue = searchBar.value;
    for (let i = 0; i < divCard.length; i++) {
      /** Boucle for afin que */
      if (
        divTitle[i].textContent
          .toLowerCase()
          .includes(searchValue.toLowerCase()) &&
        searchValue.length >= 3
      ) {
        /** Pour chaque card si le titre de notre plat inclue ce qui est saisie dans la barre de recherche */
        divCard[i].style.display =
          "block"; /** alors notre card reste ou deviennent visible */
        arrayCard.splice(divCard[i]);
        console.log(arrayCard);
      } else if (searchValue.length < 3) {
        /** et si la barre de recherche est vide  */
  ...
  searchIngredient.addEventListener("keyup", function () {
    /** Création d'une écoute sur notre barre de recherche d'ingrédients */ const inputIngredient =
      searchIngredient.value;

    for (let i = 0; i < pIngredient.length; i++) {
      /** Pour chaque ingrédient */
      if (
        pIngredient[i].textContent
          .toLowerCase()
          .includes(inputIngredient.toLowerCase())
      ) {
        /** si notre ingrédient saisie et inclus dans notre liste */
        pIngredient[i].style.display =
          "block"; /** Alors l'ingrédient reste affiché */
      } else {
      ...
      if (inputIngredient === "") {
        ...
        pIngredient[i].style.display =
          "block"; /** Alors les ingrédients restent affiché */
      }
    }
  });
  ...
  for (let iI = 0; iI < pIngredient.length; iI++ /** Pour chaque ingrédient */)
    pIngredient[iI].addEventListener("click", function () {
      /** Création d'une écoute sur nos ingrédients */ let ingredientCardArray =
        [];
      for (let i = 0; i < spanIngredient.length; i++) {
        /** Pour chaque Span contenant nos ingrédient */
        if (
          spanIngredient[i].textContent
            .toLowerCase()
            .includes(pIngredient[iI].textContent.toLowerCase())
        ) {
          /** si notre Span contient notre ingrédient */
          divCard[i].style.display =
            "block"; /** Alors notre card reste affiché */
  ...
  ingredientSearch.addEventListener("keyup", function (e) {
    /** Création d'une écoute d'évenement sur notre zone de saisie */ let ingredientEnterCardArray =
      [];
    if (
      e.key === "Enter" &&
      ingredientList.includes(ingredientSearch.value.toLowerCase())
    ) {
      for (let i = 0; i < spanIngredient.length; i++) {
        /** Pour chaque Span contenant nos ingrédient */
        if (
      ...
      if ((e.key === "Enter") & (ingredientSearch.value !== "")) {
        /** Si la touche ENTER et que la valeur de la zone de recherche n'est pas vide alors il crée un tag */
  ...
  searchAppareils.addEventListener("keyup", function () {
    /** Création d'une écoute sur notre barre de recherche d'appareils */ const inputAppareils =
  ...
  for (let iA = 0; iA < pAppareils.length; iA++ /** Pour chaque appareil */)
    pAppareils[iA].addEventListener("click", function () {
      /** Création d'une écoute sur nos appareils */ let appareilCardArray = [];
      for (let i = 0; i < cardDescription.length; i++) {
        /** Pour chaque déscription contenant nos appareils */
        if (
          cardDescription[i].textContent.includes(pAppareils[iA].textContent)
        ) {
          /** Si notre déscription contient notre appareil */
          divCard[i].style.display =
            "block"; /** Alors notre card reste affiché */
  ...
  appareilSearch.addEventListener("keyup", function (e) {
    /** Création d'une écoute d'évenement sur notre zone de saisie */ let appareilEnterCardArray =
      [];
    if (
      e.key === "Enter" &&
      appareilList.includes(appareilSearch.value.toLowerCase())
    ) {
      for (let i = 0; i < cardDescription.length; i++) {
        /** Pour chaque déscription contenant nos appareils */
        if (
          cardDescription[i].textContent.includes(
            appareilSearch.value.toLowerCase()
          )
        ) {
          /** Si notre déscription contient notre appareil */
          divCard[i].style.display =
  ...
  searchUstensiles.addEventListener("keyup", function () {
    /** Création d'une écoute sur notre barre de recherche d'ustensiles */ const inputUstensiles =
      searchUstensiles.value;
    for (let i = 0; i < pUstensils.length; i++) {
      /** Pour chaque ingrédient */
      if (pUstensils[i].textContent.includes(inputUstensiles.toLowerCase())) {
        /** si notre ustensile saisie et inclus dans notre liste */
        pUstensils[i].style.display =
          "block"; /** Alors l'ustensile reste affiché */
      } else {
      ...
      if (inputUstensiles === "") {
        /** Si la zone de saisie est vide*/
        pUstensils[i].style.display =
          "block"; /** Alors les ustensiles restent affiché */
      }
    }
  });
  ...
  for (let iU = 0; iU < pUstensils.length; iU++ /** Pour chaque ustensile */)
    pUstensils[iU].addEventListener("click", function () {
      /** Création d'une écoute sur nos ustensiles */ let ustensileCardArray =
        [];
      for (let i = 0; i < cardDescription.length; i++) {
        /** Pour chaque déscription contenant nos ustensiles */
        if (
          cardDescription[i].textContent.includes(pUstensils[iU].textContent)
        ) {
          /** Si notre déscription contient notre ustensile */
          divCard[i].style.display =
            "block"; /** Alors notre card reste affiché */
  ...
  ustensileSearch.addEventListener("keyup", function (e) {
    ...
    if (
      ...
      /**Si notre liste d'ustensile inclue l'ustensile saisie alors il crée un tag */
      for (let i = 0; i < cardDescription.length; i++) {
        /** Pour chaque déscription contenant nos appareils */
        if (
          cardDescription[i].textContent.includes(
            ustensileSearch.value.toLowerCase()
          )
        ) {
          /** Si notre déscription contient notre appareil */
          divCard[i].style.display =
...
let dishNameListe = [];
dishName.forEach((title) => {
  dishNameListe.push(title.textContent);
});
